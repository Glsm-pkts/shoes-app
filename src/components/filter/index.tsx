...
const handleReset = () => {
  ...
  return (
    <div className={
        isOpen ?
        `max-lg:fixed max-lg:inset-0 max-lg:grid max-lg:place-items-center max-lg:bg-zinc-900 max-lg:bg-opacity-60 z-10`: "max-lg:hidden"}>
      ...
      <form className="max-lg:p-5 bg-gray h-full flex flex-col gap-6 rounded-b-md">
        ...
        <Color size={color} setSize={setColor}/>
        <Gender size={gender} setSize={setGender}/>
        <Price  value={value}  setValue={setValue}/>

        <button
        onClick={handleReset}
        type="reset"
        className="border border-gray-dark p-2 rounded-lg hover:bg-dark hover:text-white transition"
        >Sıfırla</button>
      </form>
      </div>
    </div>
